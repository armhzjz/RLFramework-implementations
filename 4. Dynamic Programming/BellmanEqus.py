...
class PolicyEvaluation(metaclass=abc.ABCMeta):
    ...
    def __init__(self,
                    environment: GridWorld.Evironment,
                    gamma: Discount = 0.9,
                    policy: Dict[State: Dict[Actn: ActnProb]] or Dict[Actn: ActnProb] or None = None) -> None:
        self._environment = environment
        self._policy = \
            {s: {a: 1. / len(self._actions) for a in self._actions.values()} for s in self._num_states} if policy is None \
            else \
...
class BellmanStateValulePolEval(PolicyEvaluation):
    def __init__(self, environment: GridWorld.Evironment,
    ...
    def __getExpectedReward(self, state: PolicyEvaluation.State) -> PolicyEvaluation.reward:
        ...
        for a, ap in zip(actions.keys(), actions.values()):
            for state_prime, state_prime_p, reward in zip(self._environment.getPossibleNextStsRew(a)):
